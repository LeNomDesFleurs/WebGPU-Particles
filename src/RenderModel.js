...
export class RenderModel {
    ...
    swapFramebuffer(encoder) {
        const swapChainPass = encoder.beginRenderPass({
            colorAttachments: [
                {
                    view: this.renderCtx.getView(), // original canvas view
                    loadOp: 'clear',
                    storeOp: 'store',
                    clearValue: [0, 0, 0, 1],
                },
        ...
        swapChainPass.setPipeline(this.blitPipeline); // a pipeline that draws a full-screen quad using `this.renderTexture` as input
        swapChainPass.setBindGroup(0, this.blitBindGroup); // sampling your `this.renderTexture`
